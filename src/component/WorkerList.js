...
export default function WorkerList() {
    // fetch로 받아온 데이터를 atom 변수에 저장하여 사용
    ...
    const handleSortByStatus = () => {
        setStatusSortOrder(prevOrder => {
            ...
            if (prevOrder === 'CAUTION') return 'WARNING'; // 정렬 순서 caution -> warning
            if (prevOrder === 'WARNING') return 'SAFE'; // 정렬 순서 warning -> safe
    ...
    //     } else if (statusSortOrder === "WARNING") {
    //         return a.status === "WARNING" ? -1 : 1; // WARNING 상태가 우선
    //     } else if (statusSortOrder === "SAFE") {
    //         return a.status === "SAFE" ? -1 : 1; // SAFE 상태가 우선
    //     } else return 0; // 기본 상태(정렬 순서가 설정되지 않음)
    // });

    // Sort 방법 3) 정렬 로직: CAUTION -> WARNING -> SAFE -> CAUTION
    const getStatusPriority = (status, sortOrder) => {
        // 각 상태에 대한 우선 순위를 동적으로 정의
        const order = {
            CAUTION: sortOrder === "CAUTION" ? 1 : (sortOrder === "WARNING" ? 3 : 2),
            WARNING: sortOrder === "WARNING" ? 1 : (sortOrder === "CAUTION" ? 3 : 3),
            SAFE: sortOrder === "SAFE" ? 1 : (sortOrder === "CAUTION" ? 2 : 2),
        };
        return order[status] || 4; // 정의되지 않은 상태는 가장 낮은 우선순위
    };

    const sortedWorkers = statusSortOrder === "ALL" ? workers : [...workers].sort((a, b) => {
        const priorityA = getStatusPriority(a.status, statusSortOrder);
        const priorityB = getStatusPriority(b.status, statusSortOrder);
    
        return priorityA - priorityB; // 우선순위에 따라 정렬
    });
                    ...
                    <thead className="text-xs text-gray-700 bg-slate-100 sticky top-0">
                        <tr>
                            ...
                            <th scope="col" className="text-center w-24 h-10 hover:underline" onClick={handleSortByStatus}>
                                상태 ▼ <br /> {statusSortOrder === "CAUTION" ? "(CAUTION 우선)"
                                              : statusSortOrder === "WARNING" ? "(WARNING 우선)"
                                              : statusSortOrder === "SAFE" ? "(SAFE 우선)" : ""}
                            </th>
                        </tr>
                    </thead>
                    <tbody>
